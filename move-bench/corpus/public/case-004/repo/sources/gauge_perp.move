module dexlyn_tokenomics::gauge_perp {
    use std::bcs;
    ...
    const GAUGE_SYSTEM_SEEDS: vector<u8> = b"GAUGE_PERP_SYSTEM";
    ...
    struct EmergencyDeactivatedEvent has store, drop {
        gauge: address,
        timestamp: u64
    }
    ...
    struct GaugePerp has key {
        ...
        // The total supply of DXLP coins staked in this gauge
        total_supply: u64,
    ...
    public(friend) fun initialize(sender: &signer) {
        ...
        move_to(
            &gauge_sys_signer,
            GaugePerpSystem { owner: @owner, extend_ref }
        );
    }
    ...
    public entry fun get_reward_distribution(
        ...
        assert!(exists<GaugePerp>(gauge_address), ERROR_GAUGE_NOT_EXIST);
        let gauge = borrow_global_mut<GaugePerp>(gauge_address);
    ...
    /// * `gauge_address` - The address of gauge.
    public entry fun get_reward(user: &signer, gauge_address: address) acquires GaugePerp {
        assert!(exists<GaugePerp>(gauge_address), ERROR_GAUGE_NOT_EXIST);
    ...
    ) acquires GaugePerpSystem, GaugePerp {
        assert!(new_distribution != @0x0, ERROR_ZERO_ADDRESS);
        assert!(exists<GaugePerp>(gauge), ERROR_GAUGE_NOT_EXIST);


        let gauge_system_address = get_gauge_system_address();

        let gauge_sys = borrow_global<GaugePerpSystem>(gauge_system_address);
        assert!(address_of(owner) == gauge_sys.owner, ERROR_NOT_OWNER);

        let gauge = borrow_global_mut<GaugePerp>(gauge);
    ...
    ///
    /// # Arguments
    /// * `user` - The signer representing the user depositing DXLP coins.
    /// * `amount` - The amount of DXLP coins to deposit.
    /// * `TypeArgument` - The DXLP coin type <AssetT>.
    public entry fun deposit<AssetT>(user: &signer, amount: u64) acquires GaugePerp {
        let gauge_address = get_gauge_address_from_coin<AssetT>();
        assert!(exists<GaugePerp>(gauge_address), ERROR_GAUGE_NOT_EXIST);

        let gauge = borrow_global_mut<GaugePerp>(gauge_address);
        assert!(!gauge.emergency, ERROR_IN_EMERGENCY_MODE);

        let balance = coin::balance<DXLP<AssetT>>(address_of(user));
        assert!(balance >= amount, ERROR_INSUFFICIENT_BALANCE);

        deposit_internal<DXLP<AssetT>>(gauge, gauge_address, user, amount);
    }

    /// Withdraw a certain amount of LP coin.
    ///
    /// # Arguments
    /// * `user` - The signer representing the user withdrawing DXLP coins.
    /// * `amount` - The amount of DXLP coins to withdraw.
    /// * `TypeArgument` - The DXLP type <AssetT>.
    public entry fun withdraw<AssetT>(user: &signer, amount: u64) acquires GaugePerp {
        let gauge_address = get_gauge_address_from_coin<AssetT>();
        assert!(exists<GaugePerp>(gauge_address), ERROR_GAUGE_NOT_EXIST);
        ...
        withdraw_internal<DXLP<AssetT>>(gauge, user, amount);
    }

    /// Withdraw a certain amount of DXLP coin in emergency mode.
    ///
    /// # Arguments
    /// * `user` - The signer representing the user withdrawing DXLP coins in emergency mode.
    /// * `amount` - The amount of DXLP coins to withdraw in emergency mode.
    /// * `TypeArgument` - The DXLP type <AssetT>.
    public entry fun emergency_withdraw_amount<AssetT>(
        user: &signer,
        amount: u64
    ) acquires GaugePerp {
        ...
        *balance = *balance - amount;
        ...
        //transfer DXLP coin from gauge to user account
        supra_account::transfer_coins<DXLP<AssetT>>(
            &gauge_signer,
            user_address,
            amount
        );
        ...
        event::emit(WithdrawEvent {
            ...
            gauge: gauge_address,
            coin: gauge.coin,
            coin_type: type_info::type_name<DXLP<AssetT>>(),
            timestamp: timestamp::now_seconds()
        });
    }
    ...
    ///
    /// # Arguments
    /// * `coin` - The coin address to get gauge address.
    ///
    /// # Returns
    /// The address of the gauge for the specified DXLP coin.
    public fun get_gauge_address(coin: address): address {
    ...
    ///
    public fun check_and_get_gauge_address(coin: address): address {
        let gauge_address = get_gauge_address(coin);
        //Check gauge created
        assert!(
            exists<GaugePerp>(gauge_address),
    ...
    ///
    public fun total_supply(gauge_address: address): u64 acquires GaugePerp {
        assert!(exists<GaugePerp>(gauge_address), ERROR_GAUGE_NOT_EXIST);
        let gauge = borrow_global<GaugePerp>(gauge_address);
        gauge.total_supply
    }
    ...
    public fun balance_of(gauge_address: address, account: address): u64 acquires GaugePerp {
        assert!(exists<GaugePerp>(gauge_address), ERROR_GAUGE_NOT_EXIST);
        let gauge = borrow_global<GaugePerp>(gauge_address);
    ...
    public fun last_time_reward_applicable(gauge_address: address): u64 acquires GaugePerp, {
        assert!(exists<GaugePerp>(gauge_address), ERROR_GAUGE_NOT_EXIST);
        let gauge = borrow_global<GaugePerp>(gauge_address);
    ...
    public fun reward_per_token(gauge_address: address): u256 acquires GaugePerp {
        assert!(exists<GaugePerp>(gauge_address), ERROR_GAUGE_NOT_EXIST);
        let gauge = borrow_global<GaugePerp>(gauge_address);
    ...
    public fun earned(gauge_address: address, account: address): u64 acquires GaugePerp {
        assert!(exists<GaugePerp>(gauge_address), ERROR_GAUGE_NOT_EXIST);
        let gauge = borrow_global<GaugePerp>(gauge_address);
    ...
    public fun earned_many(gauge_addresses: vector<address>, account: address): (u64, vector<u64>) acquires GaugePerp {
    ...
    public fun reward_for_duration(gauge_address: address): u64 acquires GaugePerp {
        assert!(exists<GaugePerp>(gauge_address), ERROR_GAUGE_NOT_EXIST);
        let gauge = borrow_global<GaugePerp>(gauge_address);
    ...
    public fun period_finish(gauge_address: address): u64 acquires GaugePerp {
        assert!(exists<GaugePerp>(gauge_address), ERROR_GAUGE_NOT_EXIST);
        let gauge = borrow_global<GaugePerp>(gauge_address);
    ...
    ///
    /// # Arguments
    /// * `TypeArgument` - The type of DXLP coin
    ///
    /// # Returns
    /// * `address` - The calculated address derived from the seed.
    public fun get_dxlp_coin_address<AssetT>(): address {
        assert!(coin::is_coin_initialized<DXLP<AssetT>>(), ERROR_INVALID_COIN_TYPE);
        let coin_name = type_info::type_name<DXLP<AssetT>>();
        object::create_object_address(&SC_ADMIN, *string::bytes(&coin_name))
    }
    ...
    /// * `coin` - The coin address to create gauge address.
    ///
    public(friend) fun create_gauge(
        distribution: address,
        external_bribe: address,
        coin: address,
    ): address acquires GaugePerpSystem {
        let gauge_system_address = get_gauge_system_address();

        let seed = bcs::to_bytes(&coin);
        let new_gauge_address =
            object::create_object_address(&gauge_system_address, seed);

        // Check gauge should not exist
        assert!(
            !exists<GaugePerp>(new_gauge_address),
    ...
    // -----------------------------------------------------------------------------
    // -----------------------------------------------------------------------------
    inline fun get_gauge_address_from_coin<AssetT>(): address {
        get_gauge_address(get_dxlp_coin_address<AssetT>())
    }
    ...
    fun reward_per_token_internal(gauge: &GaugePerp): u256 {
    ...
    fun update_reward(gauge: &mut GaugePerp, account: address) {
    ...
    /// # Returns
    /// The total earned rewards for the user in the specified gauge.
    fun earned_internal(gauge: &GaugePerp, account: address): u64 {
    ...
    fun deposit_internal<AssetT>(
        ...
        *balance = *balance + amount;
        ...
        //transfer DXLP coin to gauge
        supra_account::transfer_coins<AssetT>(
            user,
            gauge_addr,
            amount
        );
        ...
        event::emit(DepositEvent {
            ...
            gauge: gauge_addr,
            coin: gauge.coin,
            coin_type: type_info::type_name<AssetT>(),
            timestamp: timestamp::now_seconds(),
        });
    }

    /// withdraw internal
    fun withdraw_internal<AssetT>(
        gauge: &mut GaugePerp, user: &signer, amount: u64
    ) {
        assert!(amount > 0, ERROR_AMOUNT_MUST_BE_GREATER_THAN_ZERO);
    ...
    #[test_only]
    public fun get_gauge_system_owner(): address acquires GaugePerpSystem {
        let gauge_system_address = get_gauge_system_address();
        borrow_global<GaugePerpSystem>(gauge_system_address).owner
    }
    ...
    public fun get_gauge_state(
        gauge_address: address
    ): (bool, address, address, address, u64, u64, u256, u64, u256, u64, u64) acquires GaugePerp {
        assert!(exists<GaugePerp>(gauge_address), ERROR_GAUGE_NOT_EXIST);
        let gauge = borrow_global<GaugePerp>(gauge_address);
